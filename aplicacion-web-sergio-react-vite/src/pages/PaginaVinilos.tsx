...
const PaginaVinilos: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchVinilos();
    const fetchUsuariosTipo = async () => {
      try {
        const usuarios = await UsuarioApi.listTipo();
        if (mounted) setUsuariosCompletos(Array.isArray(usuarios) ? usuarios : []);
        try {
          const stored = localStorage.getItem('usuarioActivo');
          if (stored) {
            const parsed: UsuarioExtendido = JSON.parse(stored);
            const existe = (usuarios || []).find((u: UsuarioExtendido) => u.dni === parsed.dni);
            if (existe) {
        ...
        console.error('Error cargando usuarios con tipo desde la API 7019:', err);
